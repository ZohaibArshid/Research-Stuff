# -*- coding: utf-8 -*-
"""
Created on Sat Jan 23 13:21:07 2021
...
'''
pd.set_option('display.max_colwidth', 3, 'display.max_rows', None)
#pd.set_option('display.max_rows', None)
df= pd.read_csv('review_sentiments_dataset.csv')
len(df)
#df.head()
#print(df)
#dataset=[]
...
class StanfordNLP:
    def __init__(self):
        self.nlp = StanfordCoreNLP('http://localhost:8811')
        self.props = {
    ...
    #def sentiment(self,sentence):
        #return self.nlp.sentiment(sentence)
...
if __name__ == '__main__':
    ...
    #print("Sentiment:", sNLP.sentiment(text))
...
result = nlp.annotate(str(text),
                  properties={
                      'annotators': 'sentiment',
                      'outputFormat': 'json',
                      'timeout': 9000000000000000000000000000000000000000000000
                   })
...
for s in result["sentences"]:
    a="{} :".format(" ".join(t["word"] for t in s["tokens"]))
    print(a)
    sentence.append(a)
    print(s["sentiment"])
    senti6.append(s["sentiment"])

'''
for sen in range(len(senti6)):
    if senti6[sen]=='Verypositive':
        print('Positive')
    elif senti6[sen]=='Verynegative':
        print('Negitive')
'''   


for i,value in enumerate(senti6):
    senti6[i] = value.replace('Verypositive', 'Positive')
    
for i,value in enumerate(senti6):
    senti6[i] = value.replace('Verynegative', 'Negitive')
...
#print(classification_report(sentence, senti6))
...
for i in range(len(senti6)):
    if senti6[i] == 'Positive':
        pos.append(senti6[i])
    elif senti6[i] == 'Negative':
        neg.append(senti6[i])
    elif senti6[i] == 'Neutral':
        neu.append(senti6[i])
...
#row = zip(sentence,senti6)
with open("sanford core nlp.csv", "w", newline= "") as csvfile:
...
#cd C:\Users\Zobi Tanoli\Anaconda3\stanford-corenlp-4.2.0
#java -mx4g -cp "*" edu.stanford.nlp.pipeline.StanfordCoreNLPServer -port 8811 -timeout 75000
